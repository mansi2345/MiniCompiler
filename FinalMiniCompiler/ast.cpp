...
string InputStmtNode::analyze(SymbolTable &symbols)
{
    ...
    {
        // Not declared yet, declare as given type
        symbols.declare(varName, this->inputType, lineNumber);
    }
    return "void";
}
...
llvm::Value *LiteralNode::codegen(CodeGenContext &context)
{
    ...
    case Type::String:
    {
        // returning a global context string it produces a pointer to a read only global memory location containing sting data
        return context.builder.CreateGlobalStringPtr(value);
        /*
                llvm::Value *strPtr = context.builder.CreateGlobalStringPtr(value);

            // Store in namedValues, using some unique name for this literal
            std::string literalName = "__str_literal_" + std::to_string(context.getNextStringLiteralID());
            context.namedValues[literalName] = strPtr;
            context.namedValues[literalName + "_raw"] = strPtr;  // same for literals

            // Return the pointer to be used immediately
            return strPtr;
        */
    }
...
llvm::Value *InputStmtNode::codegen(CodeGenContext &context)
{
    ...
    // Determine type and format string
    if (inputType == "int")
    {
    ...
    // Always allocate new memory for fresh input (prevents type mismatch bugs)
    ptr = context.builder.CreateAlloca(llvmType, nullptr, varName);

    // For string, we need to get i8* pointer from [256 x i8]*
    llvm::Value *scanfArgPtr = ptr;
    if (inputType == "string")
    {
        // Get pointer to the start of array (i8*)
        scanfArgPtr = context.builder.CreateInBoundsGEP(
            llvmType, ptr,
            {llvm::ConstantInt::get(llvm::Type::getInt32Ty(context.llvmContext), 0),
             llvm::ConstantInt::get(llvm::Type::getInt32Ty(context.llvmContext), 0)},
            "strptr");

        // Save both raw buffer and i8* if needed later
        context.namedValues[varName + "_raw"] = ptr; // [256 x i8]*
        context.namedValues[varName] = scanfArgPtr;  // i8* for use in printf/scanf
    }
    else
    {
        context.namedValues[varName] = ptr;
    ...
    // Call scanf
    llvm::Value *call = context.builder.CreateCall(scanfFunc, {formatStr, scanfArgPtr});

    // Convert int to bool if needed
    if (inputType == "bool")
    {
        ...
        llvm::Value *boolPtr = context.builder.CreateAlloca(
            llvm::Type::getInt1Ty(context.llvmContext), nullptr, varName + "_bool");

        context.builder.CreateStore(boolVal, boolPtr);
        context.namedValues[varName] = boolPtr; // override for later uses
    }
